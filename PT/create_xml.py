...
excel_table = 'Parallel_trade_adatok_2015_22_20221006.xlsx'
sheet_name = 'DOCS_20221006_WS'
csvFile = 'PT_test.csv'
xmlFile = 'PT_2022_10_06_WS_test.xml'
        ...
        if rowNum == 0:
            ...
            for i in range(len(tags)):
                ...
                if tags[i].find("file") != -1:
                    row[i] = row[i].replace('&','&amp;')
                    index_file = row[i].find(ws)+1
                    subfolder = row[i][index_file-1:]
                    subfolder = subfolder.replace(file_name,"")
                    subfolder = subfolder.replace(ws,"")
            ...
            for i in range(len(tags)):
                if tags[i].find("mime") != -1:
                    if row[i] != "":
                        ...
                        xmlData.write('\t' + '<template action="createfromtemplate">Content Server Document Templates:Egis Magyarország:Marketing &amp; Sales:Parallel Trade:Parallel Trade</template>' + "\n")
                        xmlData.write('\t' + '</otsapwksp>' + "\n")
            ...
            for i in range(len(tags)):
                if tags[i].find("attribute") != -1:
                    if len(row[i])>0:
                        if (tag_options[i].find("Célország") != -1) or (tag_options[i].find("Forrásország") != -1):
                            row[i] = row[i] + ","
                    
                        if (tag_options[i].find("Termék kereskedelmi forgalomba hozatalának dátuma") != -1):
                             if(is_date(row[i]) == False):
                                row[i] = ""
                            ...
                            if (len(set_options[i]) > 0):
                                if(set_name == set_options[i]):
                                    ...
                                    if(i==first_set):
                                        ...
                                        xmlData.write('\t\t' + '<setattribute name="'+set_options[i]+'">' + "\n")
                                        xmlData.write('\t\t\t' + '<' + tags[i] + ' name="'+tag_options[i]+'">' + row[i] + '</' + tags[i] + '>' + "\n")
                    ...
                    if len(row[i+1]) == 0:
                        ...
                        xmlData.write('\t' + '<' + tags[i+1]+'>' + f_name[index:] + '</' + tags[i+1] + '>' + "\n")
                         ...
                         # print(tags[i])
                        if ((tags[i] == "location") and (subfolder != "")):
                            xmlData.write('\t\t\t' + '<' + tags[i] + ' name="'+tag_options[i]+'">' + row[i]+":"+subfolder+ '</' + tags[i] + '>' + "\n")
                            # print("1")
                        else:
                            # print("2")
                            xmlData.write('\t\t\t' + '<' + tags[i] + ' name="'+tag_options[i]+'">' + row[i] + '</' + tags[i] + '>' + "\n")
