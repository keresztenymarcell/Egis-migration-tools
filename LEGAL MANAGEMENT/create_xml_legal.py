...
excel_table = 'OpenText jogi ügykövető migráció feldolgozott_2022.11.14.xlsx'
sheet_name = 'migr'
# sheet_name = 'WS3'
            ...
            for i in range(len(tags)):
                if tags[i].find("mime") != -1:
                    if row[i] != "":
                        ...
                        xmlData.write('\t' + '<template action="createfromtemplate">Content Server Document Templates:Egis Magyarország:Legal:Legal Case:Legal Case</template>' + "\n")
                        xmlData.write('\t' + '</otsapwksp>' + "\n")
            ...
            for i in range(len(tags)):
                ...
                if tags[i].find("attribute") != -1:
                    if len(row[i])>0:
                        ...
                        # same next category is the same
